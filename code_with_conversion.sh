...
################################################################################################################
## Filename: code_with_conversion.sh
## Created: September 07, 2021
## Author(s): Chung Kok, Muneeza Maqsood
##
## Purpose: 
##      - to execute CrypticHunter program with conversion format implemented
##          
## Instructions:  
##      - perform all tsv files in the input directory
##      - output atypical splicing events
##
## Time estimation:
##      - run 4 files and 4 mins finished
##      - roughly 1 file per min
##         
################################################################################################################
...
stage2=./script/population_splice_summary.R
bamsDir=./bams
...
do
    ...
    ## visualisation
    ## filter out unmapped reads (bit 0x2) and extract junction reads
    ## samtools view -h -F 4 ${id}_Aligned.out.sam  | awk '$6 ~ /N/ || $1 ~ /@/' | samtools view -bS - > ${id}_junction.bam
