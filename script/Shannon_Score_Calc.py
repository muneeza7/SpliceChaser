...
for i in range(len(bname)):
    ...
    if len(raw_dat_1) <= 2:
        ...
        for junc in range(len(raw_dat_1)):
            ...
            dat0["n_count"] = dat0[5].str.count("N")
            dat0["first_m"] = dat0[5].str.extract(r"([0-9]+)M").astype(int)
            dat0["last_m"] = dat0[5].str.extractall(r"([0-9]+)M").groupby(level=0).last().astype(int)
